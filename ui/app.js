...
function render(next) {
  ...
  $('description').textContent=active?'Your stopped workloads are recorded. Turn Game Mode off to restore them.':'Review what’s using resources, quiet the apps you choose, then put them back when you’re done. Works for playing or for demanding work like game development.';
  ...
  $('rows').replaceChildren(...view.snapshot.workloads.toSorted((a,b)=>priority(a)-priority(b)).map(w=>{
    ...
    if(w.blocked) row.append(text('span','KEPT RUNNING','protected-label'));
      ...
      for(const [value,label] of [['ask','Review first'],['allow','Close in Game Mode'],['keep','Always keep']]) {const option=text('option',label);option.value=value;select.append(option);}
